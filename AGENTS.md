...
1. **`catalog.json`** —— 105 条的路由表（id / type / status / 一句话 / `trigger` / `antiTrigger`）。
   先读它定位，再按需读 2-3 条全文。**它是生成物**（`collab catalog`），别手改。
2. `ROOT.md` —— 结构、权限、演化机制（最短总览）
3. `meta/pruning-policy.md` —— 条目怎么诞生、休眠、退役
4. `meta/base-contract.md` —— **冻结的基座**（目录 / kind / id / frontmatter / 链接 / 生成物）；
   动它 = 破坏性变更，需要 ADR + 迁移脚本 + `collab validate` 归零
5. **按需读取条目，不要全量读取**（上下文是预算，不是容器 —— [[A16-上下文预算法]]）：
...
> **路由优先级**：先看 `catalog.json` 里的 `trigger`（什么时候该读我），
> 命中再读全文。**没有 `trigger` 的条目（目前 8/105）说明它还没被路由过** ——
> 需要人工补，或说明它该被修剪。
