...
provenance: 2026-09-17→v7 收口≠停派；v8 基础设施——worktree 隔离 + 回执契约 + 派出会计（N 派=N 收）+ 沉默即失败；v9 冷启动分级——按实测吞吐缩尺，勿为「最大规模」负优化；v10 用 wave42 实测压出两条修正（补「基线绿再派工」、修「同路径写锁」的判据），**不叠散文**
enforced: null
---
# 极端无人托管集群：每 tick 门禁
