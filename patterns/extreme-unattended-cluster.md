...
provenance: 2026-09-17→v7 收口≠停派；v8 基础设施——worktree 隔离 + 回执契约 + 派出会计（N 派=N 收）+ 沉默即失败；v9 冷启动分级——按实测吞吐缩尺，勿为「最大规模」负优化；v10 用 wave42 实测压出两条修正（补「基线绿再派工」、修「同路径写锁」的判据），**不叠散文**；v11 用 wave52–56 实测压出「派工成本门槛」—— 小切片派工是**负吞吐**（worktree 30–70s + Task 常 Timeout ≫ 父写 <2min），限定 v7 三条强制派工
enforced: null
---
# 极端无人托管集群：每 tick 门禁
...
> **【派工前置 · 两条，缺一不可】**
> 0. **这一刀值不值得派？**（预计父 <5min 写完 → **父写，不派**；见「派工成本门槛」）
> 1. **基线绿吗？**（`mvn test` 退出码 == 0）—— **红着基线不派工**（新红不可归因）。
> 2. **worktree 重新基线到当前 HEAD 了吗？**（`git -C <wt> reset --hard <当前 HEAD>`）
>    —— worktree 是**某个 commit 的检出**，不是主工作区的副本：
>    **它看不见分岔之后的任何提交**。不重新基线 → 子代理拿到的是**过期快照**
>    （旧代码、旧 `working-memory/`、旧契约），而它**不会报错**，只会照旧的做。
>
...
> **RUNBOOK / loop「尚未接通」里，是否还有路径不冲突的可派切片？**（有 → 本 tick **必须推进**：大切片派出、**小切片父写**；不得等下一 wake）
> **本波派出 N=？已收回（SUCCESS|FAILED|SKIP|父接管）M=？**（M&lt;N → 禁止开下一波）

答完再动手。未写入 `loop.md` / `status.md` 的期望 = 未声明 = 本 tick 无效。

> **主轴负荷自问（v11 修订）**：本 tick 父是否在做「**派工成本 ≫ 实现成本**的实现」？是 → 改派或切开。
> **但小切片父写不是积压** —— 它是本模式下**正确的推进方式**（证据 6）。
...
| **派工成本门槛**（v11 · 实测补） | **派工有固定成本**：`git worktree add`（~670 文件）**30–70s**，Task 创建**常 Timeout**（子代理从未开工）。判据是**派工成本 ≪ 实现成本**：预计父 **<5 min** 能写完 → **主树父写，不建 worktree、不派 Task**；**仅当单路预计 >15 min** 才 worktree + Task。本条**限定**下面 `强制集群` / `收口≠停派` / `缺口表驱动` 三条 | 小切片派工 → 记**负吞吐**；改父写。用户看到的是"集群没在工作" |
| **强制集群**（v11 修订） | 有可并行切片时 **≥2 子代理** —— **但先过「派工成本门槛」**：小切片不派，父写 | 本 tick 无效；重派（仅当派工成本 ≪ 实现成本时） |
| **收口≠停派（v7 · v11 限定）** | 「单代理收口」**仅当**本 tick **无**路径不冲突的下一缺口；上波 ✅ 且 `RUNBOOK`/loop 仍有可并行项 → **同 tick 派出下一波（≥2）** —— 但**先过「派工成本门槛」**：大切片才派，小切片父写不算积压 | 记主轴积压；立刻补派（小切片父写**不算**积压） |
| **缺口表驱动（v7 · v11 限定）** | 每 tick 打开 `RUNBOOK`「尚未接通」或等价列表；路径沙箱不冲突的项 **按最大规模全派**（受同路径写锁 / worktree 约束）—— 同样**先过「派工成本门槛」** | 有缺口却单会话空转 → 无效 tick；**但小切片父写 = 在推进，不是空转** |
...
| **基线绿再派工**（v10 · 实测补） | 派工前目标仓全量测试**必须绿**；红基线只能派「修基线」这一路，不得并行派功能切片 | **红着派工 → 新红不可归因**，本波无效；先修基线 |
| **worktree 重新基线**（v10 · 实测补） | 派工前每棵 worktree **必须** `reset --hard <当前 HEAD>`；复用旧 worktree 时尤其 | 不重新基线 → 子代理在**过期快照**上工作且**不报错**；产出按「基于旧基线」作废 |
| **同路径写：分辨语义（v10 · 修 v6）** | **语义双写**（同一方法体/同一不变量被两路改）→ **禁**；**机械相邻行**（各改自己独立一行，如两个 `@Bean` 各自迁移）→ **可放行，父收口**。判据是**语义**不是**位置** | 语义双写 → 拒收；相邻行冲突 → 父收口，不算违规 |
...
- **不要多子代理改同一段逻辑**（重复映射 / 重复 bean / 同一方法体）——**语义双写**。
  但**别把这条读成"不许碰同一个文件"**：各改自己独立一行（如两个 `@Bean` 各自迁移）
  是**机械相邻行**，父收口即可 —— wave42 实测 2 分钟合完，禁掉它反而是负优化。

### 本节刻意**不再重复门禁表**

上面删掉了 8 条与门禁表**逐字重述**的「不要…」（派工≠交付 / commit 凭证 /
基线绿 / 收口≠停派 / worktree 隔离 / 回执契约 / 派出会计 / 政策堆砌）。

理由不是精简好看，是 [[patterns/derivation-over-copy]]：**同一规则写两处必然漂移**。
实测（2026-09-19）：`同路径写锁` 的判据在表里与本节各写一遍，
改一边忘了另一边 —— 结果它按"位置"判，**禁止了本该放行的机械相邻行**。

**要"不要做什么"，读门禁表的「违反时」列**（那列就是反面，且带强制动作）。
本节的职责只剩**表里没有的东西**：态度与嗅觉。
